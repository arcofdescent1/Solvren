/**
 * Phase 4 — Detector B3: Meeting missing after qualification (§15.2).
 */
...
const DETECTOR_KEY = "funnel.meeting_missing_after_qualification";
const VERSION = "1.0";

const QUALIFIED_STATUSES = ["qualified", "qualified_lead", "sales_qualified", "converted"];

export class MeetingMissingAfterQualificationDetector implements IDetector {
  ...
  async evaluate(ctx: DetectorContext): Promise<DetectionResult> {
    const qualSignals = ctx.signals.filter((s) => s.signal_key === "lead_status_changed" || s.signal_key === "lead_created");
    if (qualSignals.length === 0) return noFinding("none", "none");

    const window = ctx.detectorDefinition.evaluation_window_json as { sla_hours?: number };
    const slaHours = window.sla_hours ?? 48;

    for (const sig of qualSignals) {
      ...
      const qualTime = new Date(sig.signal_time).getTime();
      const cutoff = Date.now() - slaHours * 60 * 60 * 1000;
      if (qualTime > cutoff) continue;

      const hasMeeting = ctx.signals.some((s) => {
        if (s.signal_key !== "meeting_booked") return false;
        const mTime = new Date(s.signal_time).getTime();
        return mTime > qualTime && (s.primary_canonical_entity_id === entityId || s.source_ref === entityId);
      });
      if (hasMeeting) continue;

      const dedupeKey = `meeting_missing:${entityId}:${Math.floor(qualTime / 86400000)}`;
      const groupingKey = `lead:${entityId}`;
      const hoursSince = Math.round((Date.now() - qualTime) / 3600000);
      return actionableFinding(dedupeKey, groupingKey, dedupeKey, 0.8, {
        headline: `Qualified lead with no meeting for ${hoursSince}h`,
        detector_reason: "No meeting booked within SLA after qualification.",
