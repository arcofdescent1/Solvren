...
export default function MySQLIntegrationCard({ orgId, isAdmin, connected }: Props) {
  ...
  async function handleConnect() {
    ...
    setMsg(null);
    try {
      const res = await fetch("/api/integrations/mysql-readonly/connect", {
        method: "POST",
  ...
  async function handleTest() {
    ...
    setMsg(null);
    try {
      const res = await fetch("/api/integrations/mysql_readonly/test", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      <CardBody>
        <Stack gap={4}>
          <div className="font-semibold">
            {connected ? "Connected" : "Connect MySQL"}
          </div>
          {isAdmin && (
            <>
              {!connected && (
              ...
              {connected && (
                <IntegrationSetupActions orgId={orgId} provider="mysql_readonly" connected onTest={handleTest} testing={testing} />
              )}
            </>
          )}
