...
import { buildInternalOnboardingPayload } from "@/lib/internal/phase2/onboardingInternal";
...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ orgId: string }> }) {
  ...
  if (!internalHasPermission(gate.ctx.employeeRole, "internal.accounts.onboarding.view")) {
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  }
  ...
  const payload = await buildInternalOnboardingPayload(gate.ctx.admin, orgId);
