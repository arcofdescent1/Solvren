...
export async function POST(
  ...
  if (target.kind === "account") {
    ...
    if (acc.status === "connected" && !acc.disconnected_at) {
      return NextResponse.json({ ok: true, alreadyApplied: true });
    }
    const { count } = await admin
      .from("integration_credentials")
      .select("*", { count: "exact", head: true })
      .eq("integration_account_id", target.accountId);
    if (!count || count === 0) {
      return NextResponse.json({ error: "Cannot enable without stored credentials; use reconnect." }, { status: 409 });
    }
    const { error } = await updateIntegrationAccount(admin, target.accountId, {
      status: "connected",
    ...
    if (String((row as { status: string }).status) === "connected") {
      return NextResponse.json({ ok: true, alreadyApplied: true });
    }
  ...
  await logInternalAudit(admin, {
    ...
    orgId,
    action: "internal.integration.enabled",
