...
export async function POST(req: Request) {
  ...
  const body = (await req.json().catch(() => null)) as { incidentId?: string } | null;
  ...
  const { error } = await supabase.from("incidents").update({ change_event_id: null }).eq("id", inc.id);
  ...
  await auditLog(supabase, {
    ...
    entityType: "incident",
    entityId: inc.id,
    metadata: { previous_change_event_id: inc.change_event_id },
  });
