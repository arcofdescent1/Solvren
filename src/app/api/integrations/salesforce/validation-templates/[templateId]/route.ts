...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ templateId: string }> }) {
  ...
  const { data: existing } = await admin.from("salesforce_validation_templates").select("id").eq("id", templateId).eq("org_id", orgId).maybeSingle();
  ...
  await admin.from("salesforce_validation_templates").update(updates).eq("id", templateId).eq("org_id", orgId);
  await auditLog(supabase, { orgId, actorId: userRes.user.id, actorType: "USER", action: "salesforce.validation_template.updated", entityType: "salesforce_template", entityId: templateId, metadata: {} });
