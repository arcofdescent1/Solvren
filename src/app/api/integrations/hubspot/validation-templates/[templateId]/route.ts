...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ templateId: string }> }) {
  try {
    ...
    const { data: existing } = await admin.from("hubspot_validation_templates").select("id").eq("id", templateId).eq("org_id", ctx.orgId).maybeSingle();
    ...
    await admin.from("hubspot_validation_templates").update(updates).eq("id", templateId).eq("org_id", ctx.orgId);
    await auditLog(ctx.supabase, { orgId: ctx.orgId, actorId: ctx.user.id, actorType: "USER", action: "hubspot.validation_template.updated", entityType: "hubspot_template", entityId: templateId, metadata: {} });
