...
export async function POST(req: NextRequest) {
  ...
  let body: { changeId?: string; templateIds?: string[]; parameters?: Record<string, unknown> };
  try { body = (await req.json()) as typeof body; } catch { return NextResponse.json({ error: "Invalid JSON" }, { status: 400 }); }
  const orgId = req.nextUrl.searchParams.get("orgId") ?? (body as { organizationId?: string }).organizationId;
  ...
  const { data: account } = await admin.from("netsuite_accounts").select("id, account_id").eq("org_id", orgId).maybeSingle();
  ...
  for (const templateId of templateIds) {
    const { data: tmpl } = await admin.from("netsuite_validation_templates").select("query_text, template_type, enabled").eq("id", templateId).eq("org_id", orgId).maybeSingle();
  ...
  await auditLog(supabase, { orgId, actorId: userRes.user.id, actorType: "USER", action: "netsuite.validation.run.completed", entityType: "change", entityId: body.changeId ?? undefined, metadata: { runIds } });
