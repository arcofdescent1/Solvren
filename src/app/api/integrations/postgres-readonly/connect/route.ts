/**
 * Phase 3 — Postgres read-only connect: store config + credentials.
 */
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: existing } = await getAccountByOrgAndProvider(admin, ctx.orgId, "postgres_readonly");
    ...
    await admin.from("integration_source_configs").delete().eq("integration_account_id", accountId).eq("provider_key", "postgres_readonly");
    await admin.from("integration_source_configs").insert({
      org_id: ctx.orgId,
      provider_key: "postgres_readonly",
