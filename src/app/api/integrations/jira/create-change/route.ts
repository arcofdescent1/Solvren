...
export async function POST(req: NextRequest) {
  try {
    ...
    const ctx = await requireOrgPermission(parseRequestedOrgId(orgId), "change.create");
    ...
    const { data: change, error: changeErr } = await ctx.supabase
      .from("change_events")
      .insert({
        org_id: ctx.orgId,
        title,
    ...
    const { error: iaErr } = await ctx.supabase.from("impact_assessments").insert({
    ...
    await enqueueJiraIssuePropertySync(admin, ctx.orgId, change.id, {});
    await enqueueJiraCommentSync(admin, ctx.orgId, change.id, "change_created", {
      jiraIssueKey,
    });
