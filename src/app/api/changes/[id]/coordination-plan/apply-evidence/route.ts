...
import { applyEvidenceSuggestions } from "@/services/coordination/applyEvidenceSuggestions";
...
export async function POST(
  ...
  const result = await applyEvidenceSuggestions({
    supabase,
    orgId: String(change.org_id),
    changeId,
    plan: current.plan_json as Parameters<typeof applyEvidenceSuggestions>[0]["plan"],
  });
  ...
  await addTimelineEvent({
    ...
    eventType: "COORDINATION_EVIDENCE_APPLIED",
    title: "Coordination evidence suggestions applied",
    description: `${result.inserted} evidence item(s) added`,
