...
export async function GET(
  ...
  const { data: change, error: chErr } = await scopeActiveChangeEvents(supabase.from("change_events").select("id, org_id, domain, change_type, structured_change_type, systems_involved"))
    .eq("id", changeId)
    .maybeSingle();
